    ...
    <main class="container mx-auto px-4 py-8">
      ...
      <div v-else class="space-y-4">
          ...
          <div class="flex flex-col sm:flex-row sm:justify-between items-center gap-3">
            ...
            <router-link
              :to="`/orders/${order.id}`"
              class="rounded-lg border-2 border-slate-200 dark:border-slate-700 bg-transparent px-4 py-2 font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors w-full sm:w-auto text-center"
            >
              View Details
            </router-link>
          </div>
