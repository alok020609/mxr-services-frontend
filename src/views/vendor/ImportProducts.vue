  ...
  <div class="import-products-page">
    <div class="mb-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <Button variant="secondary" @click="$router.push({ name: 'VendorProducts' })">
          ← Back to Products
        </Button>
      </div>
    </div>

    <!-- Import Content (same as admin version but uses vendor API) -->
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <!-- Manual Entry Tab - Same table structure as admin -->
      <div v-if="activeTab === 'manual'" class="space-y-4">
        ...
        <!-- Table Structure - Same as admin version -->
...
const downloadTemplate = (format: 'csv' | 'json' | 'xlsx') => {
  ...
  if (format === 'csv') {
    ...
    const example = [
      ...
      'China',
      `"${manufacturerInfoExample.replace(/"/g, '""')}"`,
      'TechBrand',
      'TB-HEAD-2024',
      `"${weightDimensionsExample.replace(/"/g, '""')}"`,
      '1',
      '10',
      `"${specificationsExample.replace(/"/g, '""')}"`
    ]
...
const createAllProducts = async () => {
  ...
  if (successCount > 0) {
    ...
    router.push({ name: 'VendorProducts' })
  }
  if (failCount > 0) {
