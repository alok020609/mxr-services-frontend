    ...
    <div class="bg-gradient-to-r from-primary-600 to-primary-700 rounded-xl shadow-lg p-6 text-white">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p class="text-primary-100">Manage product categories and subcategories</p>
        </div>
      ...
      >
        <template #cell-name="{ value, row }">
          <div class="flex items-center">
            ...
            <div v-else class="flex-shrink-0 h-10 w-10 rounded-lg bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center mr-3">
...
const loadCategories = async () => {
  ...
  try {
    ...
    sendToTerminal('log', '[Admin Categories] API Response:', response)
    ...
    uiStore.showNotification('error', lastError.value)
    sendToTerminal('error', '[Admin Categories] Error loading categories:', error)
  } finally {
...
const handleCategorySubmit = async (categoryData: Omit<Category, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  try {
    ...
    if (editingCategory.value) {
      ...
      sendToTerminal('log', '[Admin Categories] Update response:', response)
      ...
      sendToTerminal('log', '[Admin Categories] Create response:', response)
    ...
    sendToTerminal('error', '[Admin Categories] Error saving category:', error)
  } finally {
    isLoading.value = false
  }
}
...
const deleteCategory = async (id: string) => {
  ...
  try {
    ...
    sendToTerminal('log', '[Admin Categories] Delete response:', response)
    ...
    sendToTerminal('error', '[Admin Categories] Error deleting category:', error)
  } finally {
    isLoading.value = false
  }
}
