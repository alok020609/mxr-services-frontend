<template>
  <div class="tax-calculator-vat-moss">
    <div class="card">
      <h3 class="text-lg font-semibold mb-4">VAT MOSS Calculation</h3>
      <form @submit.prevent="handleCalculate" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input
            ...
            label="To State/Province (Optional)"
          />
          <Input
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input
            ...
            label="To ZIP/Postal Code (Optional)"
          />
          <Input
      ...
      <div v-if="result" class="mt-6 pt-6 border-t">
        <h4 class="font-semibold mb-4">VAT MOSS Calculation Result</h4>
        <div class="bg-gray-50 rounded-lg p-4 space-y-2">
          <div class="flex justify-between">
            <span class="text-gray-600">Subtotal:</span>
            <span class="font-semibold">€{{ result.amount.toFixed(2) }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600">VAT Amount:</span>
            <span class="font-semibold text-primary-600">€{{ result.taxAmount.toFixed(2) }}</span>
          </div>
...
const handleCalculate = async () => {
  ...
  result.value = null
  try {
    const response = await complianceApi.calculateVATMOSS(form.value)
    if (response.success && response.data) {
      result.value = response.data
    } else {
      uiStore.showNotification('error', response.error || 'Failed to calculate VAT MOSS')
    }
  } catch (error: any) {
    uiStore.showNotification('error', error.message || 'Failed to calculate VAT MOSS')
  } finally {
