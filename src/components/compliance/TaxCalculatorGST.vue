<template>
  <div class="tax-calculator-gst">
    <div class="card">
      <h3 class="text-lg font-semibold mb-4">GST Calculation</h3>
      ...
      <div v-if="result" class="mt-6 pt-6 border-t">
        <h4 class="font-semibold mb-4">GST Calculation Result</h4>
...
const handleCalculate = async () => {
  ...
  result.value = null
  try {
    const response = await complianceApi.calculateGST(form.value)
    if (response.success && response.data) {
      result.value = response.data
    } else {
      uiStore.showNotification('error', response.error || 'Failed to calculate GST')
    }
  } catch (error: any) {
    uiStore.showNotification('error', error.message || 'Failed to calculate GST')
  } finally {
